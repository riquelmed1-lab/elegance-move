...
for(const marker of [
  ...
  'ELEGANCE_MOVE_SALES_MOBILE_RUNTIME_V7',
  'isSalesView',
  'decorate',
  'hideContext',
  'metricCards',
  ...
  'rawControlValue',
  'data-em-sale-qty',
  'data-em-sale-discount',
  'data-em-sale-total',
  'Finalizar venda',
  'PDV online',
  'Resumo financeiro',
  "primary.textContent='+ Nova venda'"
]) if(!js.includes(marker)) throw new Error(`Sales mobile JS incompleto: ${marker}`);

for(const forbidden of [
  "summary.classList.add('em-sale-native-summary')",
  "finalize.classList.add('em-sale-native-finalize')",
  '--em-native-sale-summary-h'
]) if(js.includes(forbidden)) throw new Error(`Resumo mobile voltou a mover/redimensionar o resumo nativo: ${forbidden}`);
...
new Function(js);
console.log('SALES_MOBILE_VERIFIED',{
  cssBytes:Buffer.byteLength(css)+Buffer.byteLength(nativeCss),
  jsBytes:Buffer.byteLength(js),
  version:8,
  legibilityFloor:true,
  compactCheckout:true,
  layoutPreserved:true,
  qtyFromNativeControls:true,
  discountFromNativeValues:true,
  modalAboveDock:true
});
