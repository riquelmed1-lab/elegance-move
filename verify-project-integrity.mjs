...
const [packageText,manifestText,productCss,mobileCss,mobileJs,nativeJs,salesJs,clientsJs,cleanupJs,passwordJs] = await Promise.all([
...
for(const marker of [
  ...
  if(!productCss.includes(marker)) throw new Error(`Proteção de overflow mobile ausente: ${marker}`);
}
