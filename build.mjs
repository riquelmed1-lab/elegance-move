...
const mustReplace = (from, to, label) => {
  if (!html.includes(from)) throw new Error(`Trecho esperado não encontrado: ${label}`);
  html = html.replace(from, to);
};

mustReplace("const KEY='em-store-db-v9', SESSION='em-store-session-v1';", "const KEY='em-cloud-cache-v1', SESSION='em-store-session-v1';", 'storage key');
mustReplace('const seed={version:9,clients:[],products:[],sales:[],expenses:[],entries:[],quotes:[]};', 'const seed={version:10,clients:[],products:[],sales:[],expenses:[],entries:[],quotes:[]};', 'seed version');
...
mustReplace(
  ...
  "async function showApp(){document.getElementById('login').classList.add('hidden');document.getElementById('app').classList.remove('hidden');render();await hydrateCloud()}",
  'showApp'
);
...
console.log('Elegance Move cloud frontend generated:', html.length, 'bytes');
