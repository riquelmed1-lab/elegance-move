...
for(const marker of ['ELEGANCE_MOVE_CLIENTS_MOBILE_V1','data-active-page="clientes"','em-clients-status','font-size:9.5px','font-size:8.5px']) if(!css.includes(marker)) throw new Error(`Clients mobile CSS incompleto: ${marker}`);
for(const forbidden of ['font-size:7px','font-size:7.4px','font-size:8px']) if(css.includes(forbidden)) throw new Error(`Clients mobile voltou a usar microtipografia inadequada: ${forbidden}`);
...
new Function(js);
console.log('CLIENTS_MOBILE_VERIFIED',{cssBytes:Buffer.byteLength(css),jsBytes:Buffer.byteLength(js),detailRuntime:'v10-hide-broken-metrics',legibilityFloor:true});
