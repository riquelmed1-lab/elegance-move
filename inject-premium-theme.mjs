...
const productImagesCssPath='public/product-images.css';
...
const [css,fidelity,hotfixCss,cleanupCss,productImagesCss,js,hotfixJs,cleanupJs,art]=await Promise.all([
  readFile(cssPath,'utf8'),readFile(fidelityPath,'utf8'),readFile(hotfixCssPath,'utf8'),readFile(cleanupCssPath,'utf8'),readFile(productImagesCssPath,'utf8'),
...
for(const marker of ['ELEGANCE_MOVE_PRODUCT_IMAGES_V1','.product-photo-editor','.product-photo-stock']) if(!productImagesCss.includes(marker)) throw new Error(`product-images.css inválido: ${marker}`);
...
html=html
  ...
  .replace(/<link[^>]+href=["']\/product-images\.css["'][^>]*>/g,'')
