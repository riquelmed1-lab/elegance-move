...
async function uploadProductPhoto(productId,file){const dataUrl=await compressProductPhoto(file),r=await fetch('/api/product-image',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({productId,dataUrl})}),data=await r.json().catch(()=>({}));if(!r.ok)throw new Error(data.error==='Image too large'?'A foto ficou muito pesada.':(data.error||'Não foi possível enviar a foto.'));return data.url}
async function deleteProductPhoto(productId){const r=await fetch('/api/product-image',{method:'DELETE',headers:{'Content-Type':'application/json'},body:JSON.stringify({productId})});if(!r.ok)throw new Error('Não foi possível remover a foto.');return true}
`;
